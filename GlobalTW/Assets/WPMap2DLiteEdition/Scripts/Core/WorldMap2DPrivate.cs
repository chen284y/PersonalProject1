...
namespace WPMF {
	...
	public partial class WorldMap2D : MonoBehaviour {
		...
		public const int MAP_UNITY_LAYER = 5; // will use the UI Layer for the culling of overlay layers
...
//		Vector3 lastDestinationPoint;
			...
			// User interaction
				...
				if (dragging) {
					if (buttonLeftPressed) {
						dragDirection = (Input.mousePosition - mouseDragStart) * 0.001f * dragSpeed;
				...
				if (allowUserZoom && (mouseIsOver || wheelAccel != 0)) {
					...
					if (wheelAccel != 0) {
						...
						if (wheelAccel >= 0.01f || wheelAccel <= -0.01f) {
							...
							Vector3 camPos = Camera.main.transform.position - (dest - Camera.main.transform.position) * wheelAccel * _mouseWheelSensitivity;
							Camera.main.transform.position = camPos;
							float minDistance = 0.01f * transform.localScale.y;
							float camDistance = (dest - Camera.main.transform.position).sqrMagnitude;
							// Last distance
							lastDistanceFromCamera = camDistance; 
							if (camDistance < minDistance) {
								Camera.main.transform.position = dest - (dest - Camera.main.transform.position).normalized * minDistance;
							} else if (camDistance > maxFrustumDistanceSqr) {
								Camera.main.transform.position = dest - (dest - Camera.main.transform.position).normalized * Mathf.Sqrt (maxFrustumDistanceSqr);
							}
							wheelAccel /= 1.15f;
						} else {
							wheelAccel = 0;
				...
				// Check boundaries
				if (transform.position != lastMapPosition || Camera.main.transform.position != lastCamPosition) {
					...
					Vector3 pos = Camera.main.WorldToViewportPoint (transform.TransformPoint (0.5f, 0, 0));
					if (pos.x < limitRight) {
						pos.x = limitRight;
						transform.position = Camera.main.ViewportToWorldPoint (pos) - transform.right * 0.5f * mapWidth;
					} else {
						pos = Camera.main.WorldToViewportPoint (transform.TransformPoint (-0.5f, 0, 0));
						if (pos.x > limitLeft) {
							pos.x = limitLeft;
							transform.position = Camera.main.ViewportToWorldPoint (pos) + transform.right * 0.5f * mapWidth;
						}
					...
					pos = Camera.main.WorldToViewportPoint (transform.TransformPoint (0, 0.5f, 0));
					if (pos.y < limitTop) {
						pos.y = limitTop;
						transform.position = Camera.main.ViewportToWorldPoint (pos) - transform.up * 0.5f * mapHeight;
					} else {
						pos = Camera.main.WorldToViewportPoint (transform.TransformPoint (0, -0.5f, 0));
	...
	#region Drawing stuff
		...
		public void Redraw () {
			...
			if (lastDistanceFromCamera == 0) 
				lastDistanceFromCamera = (transform.position - Camera.main.transform.position).sqrMagnitude;
	...
	#region Highlighting
		...
		bool GetLocalHitFromMousePos(out Vector3 localPoint) {
			...
			RaycastHit[] hits = Physics.RaycastAll (Camera.main.transform.position, ray.direction, 500, layerMask);
			if (hits.Length > 0) {
	...
	#region Internal API area
		...
		void SetDestination (Vector3 point, float duration) {
			...
			flyToEndQuaternion = Camera.main.transform.rotation;
			Plane p = new Plane(-transform.forward, transform.position);
			float distance = p.GetDistanceToPoint(Camera.main.transform.position);
//			float distance = Vector3.Distance(transform.TransformPoint(lastDestinationPoint), Camera.main.transform.position);
//			lastDestinationPoint = new Vector3( point.x , point.y );
			flyToEndLocation = Camera.main.transform.position + Camera.main.transform.forward * distance;
			flyToEndLocation -= Camera.main.transform.right * point.x * WorldMap2D.mapWidth;
			flyToEndLocation -= Camera.main.transform.up * point.y * WorldMap2D.mapHeight;
